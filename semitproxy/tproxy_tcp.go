...
// operating system
func (conn *Conn) DialOriginalDestination(dontAssumeRemote bool, machineSourceIP, virtualMachineIP *net.IP) (*net.TCPConn, error) {
	var remoteSocketAddress syscall.Sockaddr
	var err error

	if !dontAssumeRemote {
		log.Printf("Send Tcp Data from %s to virtual machine %s", conn.RemoteAddr().String(), conn.LocalAddr().String())
		remoteSocketAddress, err = tcpAddrToSocketAddr(&net.TCPAddr{IP: *(virtualMachineIP), Port: conn.LocalAddr().(*net.TCPAddr).Port})
	} else {
