...
LoadIptables() 
{
    ...
    # echo Clean Iptables
    iptables -t mangle -F KUBEVIRT_INBOUND 2>/dev/null
    iptables -t mangle -X KUBEVIRT_INBOUND 2>/dev/null
    iptables -t mangle -F KUBEVIRT_DIVERT 2>/dev/null
    iptables -t mangle -X KUBEVIRT_DIVERT 2>/dev/null
    iptables -t mangle -F KUBEVIRT_TPROXY 2>/dev/null
    iptables -t mangle -X KUBEVIRT_TPROXY 2>/dev/null
